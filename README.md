...
**In development. Not yet available.** What exists today is the skeleton: a
server that starts, migrates, reports its health and shuts down cleanly, as
one binary and as a compose file. Nothing here deploys an app yet.
...
- `GET /health/ready`: 200 or 503 from the last dependency checks (nostatus,
  ...
  uptime and each check's name, severity and state. Never error text.
...
cargo test --workspace --locked      # unit tests + accepttests against a spawned binary
